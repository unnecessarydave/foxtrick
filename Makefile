...
ifeq ($(BRANCH),dev)
BUILD_DIR = build
chrome _ : BUILD_DIR = build-dev-chrome
clean-chrome-dev: BUILD_DIR = build-dev-chrome
firefox _ : BUILD_DIR = build-dev-firefox
clean-firefox-dev _ : BUILD_DIR = build-dev-firefox
clean-dev _ : BUILD_DIR = build-dev
endif
...
ifeq ($(BRANCH),dev)
	...
	# copy manifest
	# - note this will not propagate manifest changes in dev branch
	# - do make clean-firefox-dev first
	cp -u $(MANIFEST_FIREFOX) $(BUILD_DIR)/manifest.json
