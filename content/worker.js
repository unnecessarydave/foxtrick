...
Foxtrick.util.notify.create = function(msg, source, opts) {
	...
	const IMG = chrome.runtime.getURL('content/resources/img/icon-128.png');
	...
	var updateOriginTab = function(originTab, tabOpts) {
		...
		return new Promise(function(resolve) {
			if (!tabOpts)
				console.log(`Error: notify, tabOpts is ${tabOpts}`);
	...
	var createChrome = async function() {
		var options = {
			...
			title: TITLE,
			message: msg,
			contextMessage: 'Click here to focus this tab',
		...
		return new Promise((fulfill, reject) => {
			...
			var onClosed = function onClosed(noteId) {
				...
				reject(new Error('Foxtrick timeout'));
			};
